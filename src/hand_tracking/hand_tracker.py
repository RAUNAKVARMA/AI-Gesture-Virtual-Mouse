...
class HandTracker:
    ...
    def _detect_camera_index(self) -> int:
        ...
        for idx in range(self.max_search_index):
            # Use default backend here for maximum compatibility; some
            # configurations return blank frames with CAP_DSHOW.
    ...
    def open(self, use_camera: bool = True) -> None:
        ...
        the landmarker is created; feed frames via ``process()`` from
        ``st.camera_input`` or other sources.
        """
        if use_camera:
